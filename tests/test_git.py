...
async def test_create_worktree_new_branch(git_env):
    ...
    wt_path = await git.create_worktree(git_env.repo_name, "new-feature", "cell-1")
...
async def test_create_worktree_existing_branch(git_env):
    ...
    wt_path = await git.create_worktree(git_env.repo_name, "existing-branch", "cell-2")
    assert Path(wt_path).exists()
    assert (Path(wt_path) / "file.txt").read_text() == "content"


async def test_remove_worktree(git_env):
    wt_path = await git.create_worktree(git_env.repo_name, "to-remove", "cell-3")
    assert Path(wt_path).exists()

    await git.remove_worktree(git_env.repo_name, wt_path)
    assert not Path(wt_path).exists()


async def test_is_behind_main_when_current(git_env):
    wt_path = await git.create_worktree(git_env.repo_name, "up-to-date", "cell-4")
    assert not await git.is_behind_main(wt_path)


async def test_is_behind_main_when_behind(git_env):
    # Create worktree
    wt_path = await git.create_worktree(git_env.repo_name, "behind-branch", "cell-5")

    # Add a new commit to main and push it
    git_env.checkout("main")
    git_env.add_commit("new_file.txt", "new content", "advance main")
    git_env.push("main")

    # Fetch in the worktree so it knows about the new main
...
async def test_push(git_env):
    ...
    wt_path = await git.create_worktree(git_env.repo_name, "push-test", "cell-8")
